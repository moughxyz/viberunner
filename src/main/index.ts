...
function evaluateMatcher(
  ...
  switch (matcher.type) {
    ...
    case "combined": {
      ...
      const results = matcher.conditions.map((condition) =>
        evaluateMatcher(condition, fileAnalysis)
      )
...
function findMatchingApps(
  apps: AppConfig[],
  fileAnalysis: FileAnalysis
): Array<{ app: AppConfig; priority: number }> {
    ...
    const apps = directories
      .map((dir: string) => {
        ...
        try {
          ...
          const metadata = JSON.parse(metadataContent).viberunner as AppConfig
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("load-app", async (_event, id: string) => {
    ...
    // Use the hardcoded Apps directory
    const RUNNERS_DIR = getAppsDirectory()
    const appDir = path.join(RUNNERS_DIR, appInstance.id)

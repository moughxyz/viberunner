...
async function loadVisualizers(): Promise<any[]> {
  ...
  const visualizers = fs.readdirSync(VISUALIZERS_DIR)
    .filter((dir: string) => fs.statSync(path.join(VISUALIZERS_DIR, dir)).isDirectory())
    .map((dir: string) => {
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('save-file-dialog', async (event, options: { defaultName: string, content: string | Buffer }) => {
  ...
  ipcMain.handle('open-file-dialog', async (event, options: { filters?: any[] }) => {
    const result = await dialog.showOpenDialog({
      properties: ['openFile'],

      ...
      submenu: [
        {
          ...
          click: async () => {
            const userDataPath = require$$3.app.getPath("userData");
            try {
              await require$$3.shell.openPath(userDataPath);
              console.log("Opened data directory:", userDataPath);
            } catch (error) {
              console.error("Failed to open data directory:", error);
              require$$3.dialog.showErrorBox("Error", `Failed to open data directory: ${error}`);
            }
          }
        }
      ]
...
const createWindow = () => {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    const rendererPath = isDev ? path.join(__dirname, `../renderer/${process.env.VITE_DEV_NAME}/index.html`) : path.join(__dirname, "../dist/index.html");

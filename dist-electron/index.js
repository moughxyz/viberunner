    ...
    handleRemoteCommand("REMOTE_BROWSER_GET_BUILTIN", function(event, contextId, moduleName, stack) {
      ...
      if (customEvent.returnValue === void 0) {
        if (customEvent.defaultPrevented) {
          ...
          customEvent.returnValue = require$$1$3[moduleName];
        }
      }
      return valueToMeta(event.sender, contextId, customEvent.returnValue);
    });
    ...
    case "filename-contains":
      if (matcher.substring) {
        ...
        if (matcher.extension) {
          const fileExtension = require$$1.extname(fileAnalysis.filename).toLowerCase();
...
function getAppsDirectory() {
  const appsDir = require$$1.join(require$$1$3.app.getPath("userData"), "Apps");
...
const createMenuBar = () => {
      ...
      submenu: [
        {
          ...
          click: async () => {
            ...
            try {
              await require$$1$3.shell.openPath(userDataPath);
  ...
  const menu = require$$1$3.Menu.buildFromTemplate(template);
...
require$$1$3.app.on("open-file", (event, filePath) => {
  event.preventDefault();
  const mainWindow = require$$1$3.BrowserWindow.getAllWindows()[0];
  if (mainWindow) {
    mainWindow.webContents.send("file-dropped", filePath);
  }
});
function registerIpcHandlers() {
  require$$1$3.ipcMain.removeAllListeners("get-apps");
  require$$1$3.ipcMain.removeAllListeners("load-app");
  require$$1$3.ipcMain.removeAllListeners("get-mimetype");
  require$$1$3.ipcMain.removeAllListeners("find-matching-apps");
  console.log("Registering IPC handlers...");
  require$$1$3.ipcMain.handle("get-apps", async () => {
    try {
  ...
  require$$1$3.ipcMain.handle("load-app", async (_event, id) => {
    const apps = await loadApps();
  ...
  require$$1$3.ipcMain.handle("get-mimetype", async (_event, filePath) => {
    return await getMimetype(filePath);
  });
  require$$1$3.ipcMain.handle("handle-file-drop", async (_event, filePath) => {
  ...
  require$$1$3.ipcMain.handle("find-matching-apps", async (_event, filePath) => {
    try {
      const apps = await loadApps();
      const fileAnalysis = await analyzeFile(filePath);
  ...
  require$$1$3.ipcMain.handle("close-window", async () => {
    try {
      const focusedWindow = require$$1$3.BrowserWindow.getFocusedWindow();
      if (focusedWindow) {
        focusedWindow.close();
        return { success: true };
      }
  ...
  require$$1$3.ipcMain.handle("get-platform", async () => {
    try {
  ...
  require$$1$3.ipcMain.handle("download-update", async () => {
    try {
  ...
  require$$1$3.ipcMain.handle("quit-and-install", async () => {
    try {
      main$2.autoUpdater.quitAndInstall();
      return { success: true };
    } catch (error2) {
  ...
  require$$1$3.ipcMain.handle("get-app-version", async () => {
    return {
      success: true,
      version: require$$1$3.app.getVersion()
    };
  });

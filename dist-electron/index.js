    ...
    handleRemoteCommand("REMOTE_BROWSER_GET_BUILTIN", function(event, contextId, moduleName, stack) {
      ...
      if (customEvent.returnValue === void 0) {
        if (customEvent.defaultPrevented) {
          ...
          customEvent.returnValue = require$$1$4[moduleName];
        }
      }
      return valueToMeta(event.sender, contextId, customEvent.returnValue);
    });
      ...
      submenu: [
        {
          ...
          click: async () => {
            const userDataPath = require$$1$4.app.getPath("userData");
            try {
              await require$$1$4.shell.openPath(userDataPath);
              console.log("Opened data directory:", userDataPath);
            } catch (error2) {
              console.error("Failed to open data directory:", error2);
              require$$1$4.dialog.showErrorBox(
                "Error",
...
const createWindow = () => {
  const mainWindow = new require$$1$4.BrowserWindow({
    height: 1e3,
    width: 1600,
    webPreferences: {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    const rendererPath = isDev ? require$$1$1.join(
      __dirname,
...
require$$1$4.app.on("open-file", (event, filePath) => {
  event.preventDefault();
  const mainWindow = require$$1$4.BrowserWindow.getAllWindows()[0];
  if (mainWindow) {
...
function registerIpcHandlers() {
  ...
  require$$1$4.ipcMain.handle("close-window", async () => {
    try {
      const focusedWindow = require$$1$4.BrowserWindow.getFocusedWindow();
      if (focusedWindow) {
        focusedWindow.close();
        return { success: true };
      }
  ...
  require$$1$4.ipcMain.handle("quit-and-install", async () => {
    try {
      main$2.autoUpdater.quitAndInstall();
      return { success: true };
    } catch (error2) {
  ...
  require$$1$4.ipcMain.handle("get-app-version", async () => {
    return {
      success: true,

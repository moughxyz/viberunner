...
async function selectVisualizersDirectory() {
  ...
  try {
    ...
    if (visualizerFolders.length === 0) {
      electron.dialog.showErrorBox("Invalid Directory", "The selected directory does not contain any valid visualizers.\n\nVisualizers should be folders containing a viz.json file.");
      return null;
    }
    ...
    electron.dialog.showErrorBox("Error", "Could not read the selected directory.");
    return null;
  }
}
function getUserDataPath() {
  return path.join(electron.app.getPath("userData"), "preferences.json");
}
...
async function ensureVisualizers() {
  ...
  const visualizersDir = path.join(electron.app.getPath("userData"), "visualizers");
...
electron.app.on("ready", async () => {
  ...
  if (prefs.visualizersDir && fs.existsSync(prefs.visualizersDir)) {
    ...
    if (!selectedVisualizersDir) {
      electron.dialog.showErrorBox("No Visualizers Directory", "You must select a valid visualizers directory to use the application.");
      electron.app.quit();
      return;
    }
  ...
  if (!success) {
    electron.dialog.showErrorBox("Visualizers Error", "Failed to load visualizers. Please check the selected directory and try again.");
    electron.app.quit();
    return;
  }
  registerIpcHandlers();
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("get-mimetype", async (event, filePath) => {

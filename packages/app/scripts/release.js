...
function writeReleaseData(version) {
  // Go up to workspace root, then down to packages/common/src
  const workspaceRoot = path.resolve(process.cwd(), '../../');
  const releasePath = path.resolve(workspaceRoot, 'packages/common/src/release.ts');
  ...
  const releaseContent = `// This file is auto-generated by the release script
export const release = {
  ...
  try {
    ...
    log(`✅ Updated release.ts`, 'green');
  } catch (error) {
    log(`❌ Error writing release.ts: ${error.message}`, 'red');
    process.exit(1);
  }
}
...
async function main() {
  ...
  // Update release.ts
  writeReleaseData(newVersion);

  // Git operations
  executeCommand('git add package.json ../../packages/common/src/release.ts', 'Adding package.json and release.ts to git');
